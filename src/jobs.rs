...
pub fn emergency_request_timed_out_job() {
    ...
    for mut emer in emergency_accesses {
        if emer.recovery_initiated_at.is_some() &&
            Utc::now().naive_utc() >= emer.recovery_initiated_at.unwrap() + Duration::days(emer.wait_time_days as i64) {
...
pub fn emergency_notification_reminder_job(){
    ...
    for mut emer in emergency_accesses {
        if (emer.recovery_initiated_at.is_some() &&
            Utc::now().naive_utc() >= emer.recovery_initiated_at.unwrap() + Duration::days((emer.wait_time_days as i64)-1) ) &&
            (emer.last_notification_at.is_none() || (emer.last_notification_at.is_some() && Utc::now().naive_utc() >= emer.last_notification_at.unwrap() + Duration::days(1) )){
            emer.save(&conn).expect("Cannot save emergency access on job");

            if CONFIG.mail_enabled() {

                // get grantor user to send Accepted email
                let grantor_user = User::find_by_uuid(&emer.grantor_uuid, &conn)
